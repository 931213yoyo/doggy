...
lock = _name = " 醉憶LINE BOT\n\n     \n\n 機器製作者Line ID:tk94411tk \n\n醉憶機器\n\n🇹🇼🇹🇼🇹🇼\n\n❤️❤️❤️\n\n🧡🧡🧡\n\n醉憶機器\n\n🇹🇼🇹🇼🇹🇼\n\n❤️❤️❤️\n\n🧡🧡🧡\n\n醉憶機器\n\nMade in Taiwan\n\nmake from 醉憶\n\n🇹🇼🇹🇼🇹🇼\n\n  "
...
admin=['udf244958c259f85f15daeb1d9a77c3e7',clMID]
msg_dict = {}
wait2 = {
    'readPoint':{},
    'readMember':{},
            ...
            if clMID in op.param3:
                if settings["autoJoin"] == True:
                    try:
                        ...
                        cl.sendContact(op.param1, "udf244958c259f85f15daeb1d9a77c3e7")
                    except Exception as error:
                        print(error)
            if clMID in op.param3:
        ...
        if op.type == 15:
            ...
            if settings["seeLeave"] == True:
                try:
                    ...
                    arrData = {'S':slen, 'E':elen, 'M':op.param2}
                    arr.append(arrData)
                    text += mention + "踢出去了   {} ！".format(str(group.name))
                    cl.sendMessage(op.param1,text, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
                except Exception as error:
                    print(error)
        if op.type == 17:
        ...
        if op.type == 26 or op.type == 25:
            ...
            if sender in admin:
                if text.lower() == 'h':
                    ...
                    cl.sendContact(to, "u51d85e3b10774d51d0f1064d7cac4609")
                elif text.lower() == 'bot':
                  ...
                  if msg.toType == 2:
                    ...
                    cl.sendMessage(to, "無名測試1")
                    cl.sendMessage(to, "無名測試2")
                    cl.sendMessage(to, "無名測試3")
                    cl.sendMessage(to, "無名測試4")
                    cl.sendMessage(to, "無名測試5")
                    cl.sendMessage(to, "無名測試6")
                    cl.sendMessage(to, "無名測試7")
                    cl.sendMessage(to, "無名測試8")
                    cl.sendMessage(to, "無名測試9")
                    cl.sendMessage(to, "無名測試10")
                    cl.sendMessage(to, "機器測試1")
                    cl.sendMessage(to, "機器測試2")
                    cl.sendMessage(to, "機器測試3")
                    cl.sendMessage(to, "機器測試4")
                    cl.sendMessage(to, "機器測試5")
                    cl.sendMessage(to, "速度測試1")
                    cl.sendMessage(to, "速度測試2")
                    cl.sendMessage(to, "速度測試3")
                    cl.sendMessage(to, "速度測試4")
                    cl.sendMessage(to, "速度測試5")
                    ...
                    elapsed_time = time.time()/100 - start/100
                    ...
                    try:
                        arr = []
                        owner ="u51d85e3b10774d51d0f1064d7cac4609"
                        ...
                        for mention in mentionees:
                            if clMID in mention["M"]:
                                if settings["detectMention"] == True:
                                    ...
                                    cl.sendMessage(to, "你好，我現在有事不在，等會聯絡您。[自動回覆] ")
